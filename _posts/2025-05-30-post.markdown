...
ここで，$\Delta \equiv  \{ y \ge \mathbf{0} \mid \sum_{a \in A} y_a = 1 \}$ は確率単体 (probability simplex) を表し，$\langle x, y \rangle$ は $x$ と $y$ の内積を意味する．
...
この問題は，DM が達成可能なペイオフに対する最良の上界（=最小の上界）を得ることを目的としている．明らかに，この問題の解および最適値は $\lambda^\star = \max_{a \in A} v_a$ であり，これは当初の最適化問題の最適値と一致する（=線形最適化における **強双対性** (strong duality)）．
...
すなわち，関数 $\lambda(v)$ は選好ベクトル $v$ に関する滑らかな関数であり，その勾配はロジット確率に一致する．最適値関数 $\lambda(v)$ は，定義より関数 $H$ の**凸共役** (convex conjugate) あるいは Legendre変換に他ならず，この事実が $p_a$ の $\lambda(v)$ の勾配としての表現にもつながっている．実際，この関係は凸最適化において一般的に現れるものであり，ロジットモデル以外のモデルでも最適値関数の勾配が選択確率として表現されることがある．
...
であり，これを用いると既にみたロジット選択則を得る．また，$\sup_{y \ge 0} L(y,\lambda) = \lambda^* + \eta$ であり，実際には既に $\lambda$ について解けた形で双対問題 $\min_{\lambda} \lambda^\star + \eta$ が得られる．Lagrange 乗数 $\lambda^\star$ が定数分だけ確率的選択の文脈の MEU とずれるのが気になる場合は $H(y)$ の定義を微修正すればよいが，選択確率と最適値には影響しない．
</div>
</div>
...
異なる凸関数 $H$ を用いることで，異なる選択ルールが導かれる．現実に使用されるすべての ARUM は，このような **摂動最適化問題** によって表現されるが，すべての摂動最適化問題の解に対応する ARUM が存在するとは限らない．